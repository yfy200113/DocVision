...
namespace Microsoft.MixedReality.Toolkit.Examples.Experimental.DialogTest
{
    ...
    public class DialogExampleController : MonoBehaviour
    {
        ...
        {
            Dialog myDialog = Dialog.Open(DialogPrefabSmall, DialogButtonType.Yes | DialogButtonType.No, "", "This is an example of a small dialog with a choice message for the user, placed at near interaction range", true);
            if (myDialog != null)
            {
                myDialog.OnClosed += OnClosedDialogEvent;
            }
